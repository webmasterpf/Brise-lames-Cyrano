...
$args_ag1 = $view->set_arguments(array(12));
...
if (!empty($view->result)) {
  ...
  $output = '<div id="last-rp"><h3 class="CLASS_NAME">'.$view->get_title().'</h3>' .$view->preview($viewdisplay_ag1,$args_ag1).'</div>';
...
elseif (empty($view->result)) {
    //Formatage du texte vide,ajout du titre de la vue
     $outputEmpty = '<div id="last-rp"><h3 class="CLASS_NAME">'.$view->get_title().'</h3>' .$emptyText.'<br>'.$emptyTextVue.'</div>';
